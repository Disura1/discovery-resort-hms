...
export const guestAuthService = {
  async register(fullName: string, email: string, password: string) {
    ...
    const existing = await guestsRepository.findByEmail(normalizedEmail);
    ...
    const guest = existing
      ? await guestsRepository.attachPassword(existing.id, passwordHash, fullName.trim())
  ...
  async login(email: string, password: string) {
    const normalizedEmail = normalizeEmail(email);
    const guest = await guestsRepository.findByEmail(normalizedEmail);
